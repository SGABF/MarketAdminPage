...
public class BackQnaServiceImpl implements BackQnaService {
	...
	public void update(BackQnaVO backQnaVO, int delFiles, String realPath) {
		BackQnaVO qnaVO = null;
		qnaVO = backQnaDAO.selectByIdx(delFiles);
		// 게시글 수정
		// 1. 글수정 -- 2. 파일추가 -- 3. 기존파일 삭제
		log.info("{}의 update 호출 : {}", this.getClass().getName(), backQnaVO + "\n" + delFiles + "\n" + realPath + "\n");
		// 1. 글수정
		if (backQnaVO.getBack_Qna_Name() == null && backQnaVO.getBack_Qna_Content() == null) { 
			System.out.println("qnaVO :   " + qnaVO + "\n");
			backQnaDAO.update(qnaVO);
		} if(backQnaVO.getBack_Qna_Name() != null && backQnaVO.getBack_Qna_Content() != null){
			backQnaDAO.update(backQnaVO);
		}
		if(backQnaVO.getBack_Qna_Name()==null && backQnaVO.getBack_Qna_Content()!=null) {
			backQnaDAO.updateContent(qnaVO.getBack_Qna_Idx());
		}if(backQnaVO.getBack_Qna_Content()!=null && backQnaVO.getBack_Qna_Content()==null) {
			backQnaDAO.updateName(qnaVO.getBack_Qna_Idx());
		}
